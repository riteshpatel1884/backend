...
app.post("/upload", isLoggedIn, upload.single("image"), async function(req,res){
  // In this req, multer adds 2 things 
//    1st is req.file which contains the file information like filename, path, size etc.
//  and 2nd is req.body which contains the other form data(text fields) like name, email etc.
// here 'image' is the name of the file input field in the form.
   let user = await userModel.findOne({email:req.user.email});
   user.profilepic = req.file.filename;  //default photo ko uploaded photo se replace kr do and req.file.filename is the name of the uploaded file which is stored in the uploads folder.
   await user.save();
   res.redirect("/profile");
})
